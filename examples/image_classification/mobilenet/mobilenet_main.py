...
def parse_args() -> argparse.Namespace:
    ...
    model_parser.add_argument("--model_type", type=str, choices=["small", "large"], default="large", help="MobileNet model size (default: large)")

    args = parser.parse_args()

    if args.data.exists() is False:
        raise FileNotFoundError(f"Unknown directory: {args.data}")    # fmt: on
...
def main() -> None:
    ...
    if args.model_type == "small":
        model = torchvision.models.get_model("mobilenet_v3_small", weights=_weight, num_classes=1000)
    elif args.model_type == "large":
        model = torchvision.models.get_model("mobilenet_v3_large", weights=_weight, num_classes=1000)
    else:
